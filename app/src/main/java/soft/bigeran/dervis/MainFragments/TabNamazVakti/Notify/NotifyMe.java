...
import soft.bigeran.dervis.MainActivity;
...
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_ACTIONS;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_ACTIONS_COLLAPSE;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_ACTIONS_DISMISS;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_ACTIONS_TEXT;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_COLOR;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_CONTENT_TEXT;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_CUSTOM_ID;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_LARGE_ICON;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_LED_COLOR;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_SMALL_ICON;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_TIME;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.NOTIFICATION_TITLE_TEXT;
import static soft.bigeran.dervis.MainFragments.TabNamazVakti.Notify.Notification.NotificationEntry.TABLE_NAME;
...
public class NotifyMe {
    ...
    private static void getPrayerTimesFromLocal(final Context context) {
        ...
        }, NumericConstants.REQUEST_TYPE_READ_PRAYER_TIMES, Config.countyCode);
    ...
    private static void getPrayerTimesFromServer(final Context context) {
        ...
        }, NumericConstants.REQUEST_TYPE_GET_PRAYER_TIMES_FROM_SERVER, Config.countyCode);
    ...
    private static void adjustNotificationTimes(Context context) {
        ...
        if (Config.notifBeforeImsak) {
            ...
            target = NumericConstants.PRAYER_TIME_BEFORE_IMSAK;
        ...
        if (Config.notifExactImsak) {
            ...
            target = NumericConstants.PRAYER_TIME_IMSAK;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifBeforeGunes) {
            ...
            target = NumericConstants.PRAYER_TIME_BEFORE_GUNES;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifExactGunes) {
            ...
            target = NumericConstants.PRAYER_TIME_GUNES;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifBeforeOgle) {
            ...
            target = NumericConstants.PRAYER_TIME_BEFORE_OGLE;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifExactOgle) {
            ...
            target = NumericConstants.PRAYER_TIME_OGLE;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifBeforeIkindi) {
            ...
            target = NumericConstants.PRAYER_TIME_BEFORE_IKINDI;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifExactIkindi) {
            ...
            target = NumericConstants.PRAYER_TIME_IKINDI;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifBeforeAksam) {
            ...
            target = NumericConstants.PRAYER_TIME_BEFORE_AKSAM;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifExactAksam) {
            ...
            target = NumericConstants.PRAYER_TIME_AKSAM;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifBeforeYatsi) {
            ...
            target = NumericConstants.PRAYER_TIME_BEFORE_YATSI;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
        ...
        if (Config.notifExactYatsi) {
            ...
            target = NumericConstants.PRAYER_TIME_YATSI;
            mapNotification(context, target, nextNotifTimeInMillis);
        }
    ...
    private static String getKeyFromTarget(Context context, int target) {
        ...
        if (target == NumericConstants.PRAYER_TIME_IMSAK) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[0] + " " + resources.getString(R.string.time);
            return StringConstants.key_prayerTimeImsak;
        } else if (target == NumericConstants.PRAYER_TIME_GUNES) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[1] + " " + resources.getString(R.string.time);
            return StringConstants.key_prayerTimeGunes;
        } else if (target == NumericConstants.PRAYER_TIME_OGLE) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[2] + " " + resources.getString(R.string.time);
            return StringConstants.key_prayerTimeOgle;
        } else if (target == NumericConstants.PRAYER_TIME_IKINDI) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[3] + " " + resources.getString(R.string.time);
            return StringConstants.key_prayerTimeIkindi;
        } else if (target == NumericConstants.PRAYER_TIME_AKSAM) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[4] + " " + resources.getString(R.string.time);
            return StringConstants.key_prayerTimeAksam;
        } else if (target == NumericConstants.PRAYER_TIME_YATSI) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[5] + " " + resources.getString(R.string.time);
            return StringConstants.key_prayerTimeYatsi;
        } else if (target == NumericConstants.PRAYER_TIME_BEFORE_IMSAK) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[0] + " " + resources.getString(R.string.time)
                    + " - " + Config.timeBeforeImsak + " " + resources.getString(R.string.minutesRemain);
            return StringConstants.key_prayerTimeBeforeImsak;
        } else if (target == NumericConstants.PRAYER_TIME_BEFORE_GUNES) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[1] + " " + resources.getString(R.string.time)
                    + " - " + Config.timeBeforeGunes + " " + resources.getString(R.string.minutesRemain);
            return StringConstants.key_prayerTimeBeforeGunes;
        } else if (target == NumericConstants.PRAYER_TIME_BEFORE_OGLE) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[2] + " " + resources.getString(R.string.time)
                    + " - " + Config.timeBeforeOgle + " " + resources.getString(R.string.minutesRemain);
            return StringConstants.key_prayerTimeBeforeOgle;
        } else if (target == NumericConstants.PRAYER_TIME_BEFORE_IKINDI) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[3] + " " + resources.getString(R.string.time)
                    + " - " + Config.timeBeforeIkindi + " " + resources.getString(R.string.minutesRemain);
            return StringConstants.key_prayerTimeBeforeIkindi;
        } else if (target == NumericConstants.PRAYER_TIME_BEFORE_AKSAM) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[4] + " " + resources.getString(R.string.time)
                    + " - " + Config.timeBeforeAksam + " " + resources.getString(R.string.minutesRemain);
            return StringConstants.key_prayerTimeBeforeAksam;
        } else if (target == NumericConstants.PRAYER_TIME_BEFORE_YATSI) {
            Resources resources = context.getResources();
            content = resources.getStringArray(R.array.prayerTimes)[5] + " " + resources.getString(R.string.time)
                    + " - " + Config.timeBeforeYatsi + " " + resources.getString(R.string.minutesRemain);
            return StringConstants.key_prayerTimeBeforeYatsi;
        } else {
            return "undefined";
        }
    ...
    public static int getSound(Context context, String key) {
        ...
        if (key.equals(StringConstants.key_prayerTimeBeforeImsak)) {
            soundId = Config.melodyBeforeImsak;
        } else if (key.equals(StringConstants.key_prayerTimeImsak)) {
            soundId = Config.melodyImsak;
        } else if (key.equals(StringConstants.key_prayerTimeBeforeGunes)) {
            soundId = Config.melodyBeforeGunes;
        } else if (key.equals(StringConstants.key_prayerTimeGunes)) {
            soundId = Config.melodyGunes;
        } else if (key.equals(StringConstants.key_prayerTimeBeforeOgle)) {
            soundId = Config.melodyBeforeOgle;
        } else if (key.equals(StringConstants.key_prayerTimeOgle)) {
            soundId = Config.melodyOgle;
        } else if (key.equals(StringConstants.key_prayerTimeBeforeIkindi)) {
            soundId = Config.melodyBeforeIkindi;
        } else if (key.equals(StringConstants.key_prayerTimeIkindi)) {
            soundId = Config.melodyIkindi;
        } else if (key.equals(StringConstants.key_prayerTimeBeforeAksam)) {
            soundId = Config.melodyBeforeAksam;
        } else if (key.equals(StringConstants.key_prayerTimeAksam)) {
            soundId = Config.melodyAksam;
        } else if (key.equals(StringConstants.key_prayerTimeBeforeYatsi)) {
            soundId = Config.melodyBeforeYatsi;
        } else if (key.equals(StringConstants.key_prayerTimeYatsi)) {
